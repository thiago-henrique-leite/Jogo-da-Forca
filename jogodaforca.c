...
void modo_de_jogo() {
    printf("\n\t  BEM-VINDO(A)!\n");
    printf("\n\t  Digite <1> para jogar com uma palavra aleatória");
    printf("\n\t  Digite <2> para entrar com uma palavra e jogar com um amigo");
    printf("\n\n\t  Insira a opção desejada e pressione <enter>: ");
    scanf("%d", &modoJogo);
}
...
void escolha_da_palavra() {
    abertura();
    if(modoJogo == 2) {
        printf("\n\t  Observação: Não inserir acentos e separar palavras compostas com um traço < - > Não utilizar espaços.\n");
...
int main() {
    ...
    printf("\n\n\tOBRIGADO POR JOGAR, VOLTE SEMPRE!\t\t\t\t\t\t\tThiago Henrique\n");
}
